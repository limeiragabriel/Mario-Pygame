...
class OverheadInfo(object):
    ...
    def __init__(self, game_info, state):
        ...
        self.coin_total = game_info[c.TOTAL_MOEDAS]
        self.time = 401
        self.current_time = 0
        self.total_lives = game_info[c.VIDAS]
        self.top_score = game_info[c.MAIOR_PONTUACAO]
        self.state = state
    ...
    def handle_level_state(self, level_info):
        ...
        if self.state == c.MAIN_MENU:
            self.score = level_info[c.PONTOS]
            ...
            self.flashing_coin.update(level_info[c.TEMPO_ATUAL])

        elif self.state == c.TELA_CARREGAMENTO:
            self.score = level_info[c.PONTOS]
            self.update_score_images(self.score_images, self.score)
            self.update_coin_total(level_info)
            ...
            self.score = level_info[c.PONTOS]
            self.update_score_images(self.score_images, self.score)
            if level_info[c.ESTADO_NIVEL] != c.CONGELADO \
        ...
        elif self.state == c.CONTAGEM_REGRESSIVA:
            level_info[c.PONTOS] += 50
            self.score = level_info[c.PONTOS]
            self.update_count_down_clock(level_info)
            self.update_score_images(self.score_images, self.score)
            self.update_coin_total(level_info)
    ...
    def update_count_down_clock(self, level_info):
        """===================================================="""
        if self.state == c.CONTAGEM_REGRESSIVA:
            self.time -= 1

        elif (level_info[c.TEMPO_ATUAL] - self.current_time) > 400:
            self.current_time = level_info[c.TEMPO_ATUAL]
            self.time -= 1
        self.count_down_images = []
        self.create_label(self.count_down_images, str(self.time), 645, 55)
    ...
    def update_coin_total(self, level_info):
        """===================================================="""
        self.coin_total = level_info[c.TOTAL_MOEDAS]
